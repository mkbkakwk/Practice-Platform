...
- **教学分析**：根据真实比赛和提交数据统计学习情况，访问范围受角色与资源权限约束。
...
正式部署包含 PostgreSQL 数据库、RabbitMQ 消息服务、Backend 后端、Worker 评测任务处理器、Runner 沙箱执行服务、Frontend 前端六个服务。**只有 Runner 可以挂载 Docker socket**；学生不直接访问数据库、消息队列或评测服务。Office 文档使用独立持久卷，不能仅备份数据库。

文档中保留服务名、命令和配置变量的英文写法，便于对照代码和终端。Outbox 指与提交一起写入数据库的待发送事件表；OPS 指运维检查使用的配置和凭据；T1 指本次维护停写后新建的配对恢复点。CI 是自动化测试，PR 是代码／文档合并请求。发布提交点是确认新版运行状态通过验收的时刻，不是 Git 提交。

## 谁来使用

| 角色 | 主要工作 |
| --- | --- |
| 管理员（ADMIN） | 管理账号与角色、查看系统状态、组织运维 |
| 教师（TEACHER） | 创建题目、Office 练习和比赛，查看教学结果 |
| 学生（源码角色 `USER`） | 注册／登录、参与练习和比赛、查看自己的提交 |

## 快速本地体验：仅限开发／评估

准备 Git、Docker 和 Docker Compose 插件。在**没有生产环境的独立开发电脑或虚拟机**运行：
...
**这些是开发命令，不是学校正式发布命令。** 开发 Compose 含默认凭据、构建指令和固定容器名；仅更换 `-p` 不能避免与生产环境的容器名冲突。不要把开发默认配置开放给校园用户。

## 学校正式部署

请从 [学校部署指南](docs/SCHOOL_DEPLOYMENT.md) 开始。正式环境使用 [docker-compose.release.yml](docker-compose.release.yml)，使用已核验的固定版本本地镜像和既有外部持久卷；不在维护窗口临时构建，不从移动分支 HEAD 随意发布。
...
| 比赛与分析 | [比赛核心](docs/contest-core.md)、[计分](docs/contest-scoring.md)、[教学分析](docs/contest-analytics.md) |
...
生产环境密钥只能保存在 Git 外的受限文件或学校批准的密钥系统中；[.env.production.example](.env.production.example) 只是变量示例，不能原样使用。不要在 Issue、日志或截图中公开密码、JWT、Runner 令牌或完整环境配置。

校园访问通过反向代理；互联网访问必须使用 HTTPS、防火墙与最小暴露端口。`PROMOTE_FIRST_ADMIN` 在日常生产环境必须关闭。清空 RabbitMQ 队列不是日常恢复手段；提交点之后不允许自动回退旧 V4 数据库。威胁模型与漏洞报告方式见 [SECURITY.md](SECURITY.md)，不要公开可利用细节或用户数据。

## 容量声明

当前版本已通过生产环境小范围受控试点，适用于日常课堂／校园使用；**尚未认证大规模同时比赛容量**。评测架构支持多个 Worker 竞争消费，但当前正式 Compose 固定单个 Worker 容器名，扩容需要单独规划和演练，不能直接套用开发环境的 `--scale` 命令。

实际并发容量取决于 CPU、内存、语言、测试用例、提交峰值与 Runner 资源限制。数百人同时提交评测之前，应进行贴近真实课程负载的容量与压力测试，不作未经测量的数字承诺。

## 开发与贡献

以真实反馈、缺陷、容量需求或明确功能请求提出 Issue，再经功能／修复分支、Docker 测试、PR、集成分支／预发布环境验收和正式发布流程推进。不要自动开启另一个编号开发阶段。

测试入口见 [scripts](scripts/) 和 [Docker 测试拓扑](docker-compose.test.yml)，CI 合同见 [ci.yml](.github/workflows/ci.yml)。测试与预发布环境必须使用隔离数据，不能指向生产环境。文档和代码变更均通过 PR 审查，不在已标记发布上静默修改。

## 许可证

[MIT 许可证原文](LICENSE)；另附[中文参考译文](docs/LICENSE.zh-CN.md)，便于阅读，不替代原文。
