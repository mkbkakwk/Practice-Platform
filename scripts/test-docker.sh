...
if [[ $build_rc -eq 0 && $release_config_rc -eq 0 ]]; then
  echo "==> Running read-only operational check fixtures"
  ops_check_rc=0
  bash ./scripts/test-ops-check.sh || ops_check_rc=$?
fi
  ...
  if [[ $docker_sandbox_security_rc -eq 0 ]]; then
    ...
    if [[ $worker_scale_rc -eq 0 ]]; then
      ...
      if [[ $judge_reliability_rc -eq 0 ]]; then
        ...
        if [[ $contest_core_rc -eq 0 ]]; then
          echo "==> Running isolated backup, restore, retention, and disk-protection drill"
          backup_restore_rc=0
          bash ./scripts/test-backup-restore.sh || backup_restore_rc=$?
        fi
      fi
    fi
  fi
...
for rc in "$build_rc" "$release_config_rc" "$release_logging_rc" "$startup_rc" "$backend_rc" "$worker_rc" \
  ...
  "$readiness_timeout_rc" "$docker_sandbox_security_rc" "$worker_scale_rc" "$ops_check_rc"; do
  if [[ $rc -ne 0 ]]; then
    exit "$rc"
  fi
done
...
if [[ $backup_restore_rc -ne 0 ]]; then
  exit "$backup_restore_rc"
fi
