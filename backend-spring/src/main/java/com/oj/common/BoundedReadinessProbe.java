...
/**
 * Runs a dependency probe outside an HTTP request thread with a deliberately
 * small, bounded execution budget. A stuck dependency can consume at most one
 * daemon probe thread; later readiness calls fail closed instead of queuing.
 */
@Component
public class BoundedReadinessProbe implements AutoCloseable {
    ...
    public BoundedReadinessProbe(Duration timeout) {
        ...
        ThreadFactory threadFactory = task -> {
            Thread thread = new Thread(task, "backend-readiness-probe-" + sequence.incrementAndGet());
